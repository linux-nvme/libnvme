...
static int test_ctrl(nvme_ctrl_t c)
{
	...
	nvme_link_t l = nvme_ctrl_get_link(c);
	int ret, temp;
	...
	ret = nvme_get_log_smart(l, NVME_NSID_ALL, true, &smart);
	if (ret) {
	...
	printf("  SMART: Current temperature:%d percent used:%d%%\n", temp,
		smart.percent_used);
	ret = nvme_get_log_sanitize(l, true, &sanlog);
	if (!ret)
		printf("  Sanitize Log:\n");
	else
		printf("  ERROR: Sanitize Log:%x\n", ret);
	ret = nvme_get_log_reservation(l, true, &resvnotify);
	if (!ret)
	...
	ret = nvme_get_log_endurance_group(l, 0, &eglog);
	if (!ret)
...
static int test_namespace(nvme_ns_t n)
{
	...
	nvme_link_t l = nvme_ns_get_link(n);
	...
	ret = nvme_identify_allocated_ns(l, nsid, &allocated);
	if (!ret)
	...
	ret = nvme_identify_ns_descs(l, nsid, descs);
	if (!ret)
		printf("  Identify NS Descriptors\n");
	else
		printf("  ERROR: Identify NS Descriptors:%x\n", ret);
	free(descs);
	ret = nvme_get_features_write_protect(l, nsid,

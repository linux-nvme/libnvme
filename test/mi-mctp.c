...
enum AemFailureCondition {
	AemFcNone,
	AemFcBadGetConfigHeaderLen,
	AemFcBadGetConfigTotalLen,
	AemFcBadGetConfigBufferLen,
	AemFcBadOccRspHdrLen_Sync,
	AemFcBadOccRspTotalLen_Sync,
	AemFcBadOccRspBufferLen_Sync,
	AemFcBadOccRspHdrLen_Aem,
	AemFcBadOccRspTotalLen_Aem,
	AemFcBadOccRspBufferLen_Aem,
};

struct aem_rcv_enable_fn_data {
	enum AemEnableState state;
	enum AemFailureCondition fc;
	...
	struct aem_rcv_enable_fn_data *fn_data, struct nvme_mi_aem_enabled_map *to_send)
{
	...
	for (int i = 0; i < 255; i++) {
		if (fn_data->events[i] && to_send->enabled[i]) {
			...
			if ((fn_data->fc == AemFcBadOccRspHdrLen_Sync && !aem_not_ack) ||
			    (fn_data->fc == AemFcBadOccRspHdrLen_Aem && aem_not_ack))
				data->aelhlen--;
			...
			if ((fn_data->fc == AemFcBadOccRspTotalLen_Sync &&
			     !aem_not_ack) ||
			    (fn_data->fc == AemFcBadOccRspTotalLen_Aem &&
			     aem_not_ack))
				aeoltl -= 1;
	...
	nvme_mi_aem_aeolli_set_aeoltl(aeoltl, list_hdr);
	test_peer.tx_buf_len = hdr_len + aeoltl;

	if ((fn_data->fc == AemFcBadOccRspBufferLen_Sync && !aem_not_ack) ||
		(fn_data->fc == AemFcBadOccRspBufferLen_Aem && aem_not_ack))
...
static int aem_rcv_enable_fn(struct test_peer *peer, void *buf, size_t len, int sd)
{
	...
	case AemEnableGetEnabled:
	{
		...
		if (fn_data->fc == AemFcBadGetConfigHeaderLen)
			list_hdr->aeslhl =
		...
		for (int i = 0; i < 256; i++) {
			if (fn_data->ep_enabled_map.enabled[i]) {
				list_hdr->numaes++;
				nvme_mi_aem_aesi_set_aesid(i, &item[item_count]);
				nvme_mi_aem_aesi_set_aee(1, &item[item_count]);
		...
		if (fn_data->fc == AemFcBadGetConfigTotalLen)
			list_hdr->aest--;//Shrink

		test_peer.tx_buf_len =
			sizeof(struct nvme_mi_mi_resp_hdr) + list_hdr->aest;
		if (fn_data->fc == AemFcBadGetConfigBufferLen)
			test_peer.tx_buf_len--;

		test_set_tx_mic(&test_peer);

		fn_data->state = AemSetToDisabled;
		break;
	}
	case AemSetToDisabled:
	{
		assert(sd == peer->sd[TEST_PEER_SD_COMMANDS_IDX]);
		...
		list_hdr->aeolhl = sizeof(*list_hdr);
		list_hdr->numaeo = 0;
		nvme_mi_aem_aeolli_set_aeoltl(list_hdr->aeolhl, list_hdr);
		...
		fn_data->state = AemEnableSetEnabled;
		break;
	}
	case AemEnableSetEnabled:
		assert(sd == peer->sd[TEST_PEER_SD_COMMANDS_IDX]);
...
{
	struct aem_rcv_enable_fn_data *fn_data = (struct aem_rcv_enable_fn_data *)userdata;
	...
	struct aem_rcv_enable_fn_data *fn_data)
{
	...
	fn_data->state = AemEnableGetEnabled;//This is the flow for disabling
	assert(nvme_mi_aem_disable(ep) == 0);
}

static void test_mi_aem_ep_based_failure_helper(nvme_mi_ep_t ep,
	enum AemFailureCondition fc, struct test_peer *peer)
{
	...
	fn_data.fc = fc;
	switch (fc) {
	case AemFcBadGetConfigHeaderLen:
	case AemFcBadGetConfigTotalLen:
	case AemFcBadGetConfigBufferLen:
	case AemFcBadOccRspHdrLen_Sync:
	case AemFcBadOccRspTotalLen_Sync:
	case AemFcBadOccRspBufferLen_Sync:
		//These all should fail before processing
		assert(nvme_mi_aem_enable(ep, false, false, false, 1, 2, &config, &fn_data) == -1);
		assert(errno == EPROTO);
		break;
	case AemFcBadOccRspHdrLen_Aem:
	case AemFcBadOccRspTotalLen_Aem:
	case AemFcBadOccRspBufferLen_Aem:
		//These should fail on the processing
		assert(nvme_mi_aem_enable(ep, false, false, false, 1, 2, &config, &fn_data) == 0);
...
{
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadGetConfigHeaderLen, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadGetConfigTotalLen, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadGetConfigBufferLen, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspHdrLen_Sync, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspHdrLen_Aem, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspTotalLen_Sync, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspTotalLen_Aem, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspBufferLen_Sync, peer);
	test_mi_aem_ep_based_failure_helper(ep, AemFcBadOccRspBufferLen_Aem, peer);
}
...
static void test_mi_aem_enable_invalid_usage(nvme_mi_ep_t ep, struct test_peer *peer)
{
	...
	assert(nvme_mi_aem_enable(ep, false, false, false, 2, 1, &config, NULL) == -1);
	...
	assert(nvme_mi_aem_enable(ep, false, false, false, 2, 1, NULL, NULL) == -1);

	config.aem_handler = aem_handler;
	config.enabled_map.enabled[0] = true;

	//Call with invalid endpoint
	assert(nvme_mi_aem_enable(NULL, false, false, false, 2, 1, &config, NULL) == -1);
}

					 ...
					 void *data)
{
	...
	/* Check cdw0: SCTLID = 0, MAXRENT = 0 (1 entry), ALL=1, INCF=1, INCPF=1, INCVF=1 */
	assert(buf[8] == 0x00);
	assert(buf[9] == 0x00);
	assert(buf[10] == 0x00);
	assert(buf[11] == 0x87);
	...
	chs->spare = 88;
	chs->cwarn = 0;
	chs->chsc[0] = NVME_MI_CHSC_RDY & 0xff;
	chs->chsc[1] = 0;
...
static void test_mi_ctrl_health_single(nvme_mi_ep_t ep)
{
	...
	rc = nvme_mi_mi_controller_health_status_poll_all(ep, false, &entry, &count);
	assert(rc == 0);
	assert(count == 1);
					...
					void *data)
{
	...
	/* Check cdw1: clear=1 (bit 31), filter_cwarn=1 (bit 4), filter_csts=1 (bit 0) */
	assert(buf[12] == 0x11);
	assert(buf[13] == 0x00);
	assert(buf[14] == 0x00);
	assert(buf[15] == 0x80);
...
static void test_mi_ctrl_health_flags(nvme_mi_ep_t ep)
{
	...
	rc = nvme_mi_mi_controller_health_status_poll(ep, &args);
	assert(rc == 0);
	assert(count == 0);
}
...
static void test_mi_ctrl_health_autopaging(nvme_mi_ep_t ep)
{
	...
	rc = nvme_mi_mi_controller_health_status_poll_all(ep, false, entries, &count);
	assert(rc == 0);
	assert(count == 300);
...
static void test_mi_ctrl_health_errors(nvme_mi_ep_t ep)
{
	...
	/* NULL args */
	rc = nvme_mi_mi_controller_health_status_poll(ep, NULL);

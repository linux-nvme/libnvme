...
static int test_mi_ctrl_health_single_cb(struct nvme_mi_ep *ep,
					 struct nvme_mi_req *req,
					 struct nvme_mi_resp *resp,
					 void *data)
{
	...
	assert(req->hdr_len == sizeof(struct nvme_mi_mi_req_hdr));
	buf = (void *)req->hdr;
	assert(buf[4] == nvme_mi_mi_opcode_ctrl_health_status_poll);
	...
	assert(resp->hdr_len >= sizeof(struct nvme_mi_mi_resp_hdr));
	assert(resp->data_len >= sizeof(*chs));
	...
	chs->spare = 88;
	chs->cwarn = 0;
	chs->chscf[0] = NVME_MI_CHSCF_RDY & 0xff;
	chs->chscf[1] = 0;
...
static void test_mi_ctrl_health_single(nvme_mi_ep_t ep)
{
	...
	rc = nvme_mi_mi_controller_health_status_poll_all(
		ep, false, &entry, &count);
	assert(rc == 0);
	assert(count == 1);
	assert(le16_to_cpu(entry.ctlid) == 0x42);
	assert(le16_to_cpu(entry.csts) == NVME_MI_CSTS_RDY);
	assert(le16_to_cpu(entry.ctemp) == 310);
	assert(entry.pdlu == 12);
	assert(entry.spare == 88);
	assert(entry.chscf[0] == (NVME_MI_CHSCF_RDY & 0xff));
}

static int test_mi_ctrl_health_flags_cb(struct nvme_mi_ep *ep,
					struct nvme_mi_req *req,
					struct nvme_mi_resp *resp,
					void *data)
{
...
static void test_mi_ctrl_health_flags(nvme_mi_ep_t ep)
{
	struct nvme_mi_ctrl_health_status entries[5] = { 0 };
	unsigned int count = 5;
	__u32 dw0_flags = NVME_MI_CHSP_DW0_INCF;
	__u32 dw1_flags = NVME_MI_CHSP_DW1_CWARN | NVME_MI_CHSP_DW1_CSTS;
	int rc;

	test_set_transport_callback(ep, test_mi_ctrl_health_flags_cb, NULL);
	rc = nvme_mi_mi_controller_health_status_poll(
		ep, 0x0102, true, dw0_flags, dw1_flags, entries, &count);
	assert(rc == 0);
	assert(count == 0);
}

static int test_mi_ctrl_health_autopaging_cb(struct nvme_mi_ep *ep,
					     struct nvme_mi_req *req,
					     struct nvme_mi_resp *resp,
					     void *data)
{
	...
	if (pd->call_count == 0) {
		/* First chunk: start_ctrl_id = 0, MAXRENT = 254 (255 entries) */
		assert(buf[8] == 0x00 && buf[9] == 0x00);
		assert(buf[10] == 254);
		num = 255;
	} else if (pd->call_count == 1) {
		/* Second chunk: start_ctrl_id = 255, MAXRENT = 44 (45 entries) */
		assert(buf[8] == 255 && buf[9] == 0x00);
		assert(buf[10] == 44);
		num = 45;
	} else {
		assert(false);
...
static void test_mi_ctrl_health_autopaging(nvme_mi_ep_t ep)
{
	...
	rc = nvme_mi_mi_controller_health_status_poll_all(
		ep, false, entries, &count);
	assert(rc == 0);
	assert(count == 300);
	assert(pd.call_count == 2);
	assert(le16_to_cpu(entries[0].ctlid) == 0);
	assert(le16_to_cpu(entries[254].ctlid) == 254);
	assert(le16_to_cpu(entries[255].ctlid) == 255);
	assert(le16_to_cpu(entries[299].ctlid) == 299);
}

static int test_mi_ctrl_health_mismatch_cb(struct nvme_mi_ep *ep,
					   struct nvme_mi_req *req,
					   struct nvme_mi_resp *resp,
					   void *data)
{
...
static void test_mi_ctrl_health_errors(nvme_mi_ep_t ep)
{
	...
	/* NULL entries */
	rc = nvme_mi_mi_controller_health_status_poll(
		ep, 0, false, 0, 0, NULL, &count);
	assert(rc == -1 && errno == EINVAL);

	/* NULL num_entries */
	rc = nvme_mi_mi_controller_health_status_poll(
		ep, 0, false, 0, 0, &entry, NULL);
	assert(rc == -1 && errno == EINVAL);

	/* zero num_entries */
	count = 0;
	rc = nvme_mi_mi_controller_health_status_poll(
		ep, 0, false, 0, 0, &entry, &count);
	assert(rc == -1 && errno == EINVAL);

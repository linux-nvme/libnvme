 ...
 *
 * This file is used to test private functions defined in "src/nvme/util.c"
 */
...
static bool test_get_entity_name() {
	size_t len;
	char entity_name[100];

	len = get_entity_name(entity_name, sizeof(entity_name));
	printf("Entity name/length: %s/%lu\n", entity_name, len);

	return true;
}

static bool test_get_entity_version() {
	size_t len;
	char entity_version[256];

	len = get_entity_version(entity_version, sizeof(entity_version));
	printf("Entity version/length: %s/%lu\n", entity_version, len);
...
int main(int argc, char *argv[]) {
	...
	printf("\n------------------------------------------------------------------------------\n");
	pass = test_get_entity_name();
	printf("get_entity_name() %s\n", pass ? "[PASS]" : "[FAIL]");
	if (!pass)
		exit_val = EXIT_FAILURE;

	printf("\n------------------------------------------------------------------------------\n");
	pass = test_get_entity_version();
	printf("get_entity_version() %s\n", pass ? "[PASS]" : "[FAIL]");
	if (!pass)
		exit_val = EXIT_FAILURE;

...
_Static_assert(sizeof(struct ae_supported_list_header_t) == 5,
	"size_of_ae_supported_list_header_t_is_not_5_bytes");
...
_Static_assert(sizeof(struct ae_supported_item_t) == 3,
	"size_of_ae_supported_item_t_is_not_3_bytes");
...
_Static_assert(sizeof(struct ae_enable_item_t) == 3,
...
_Static_assert(sizeof(struct ae_enable_list_header_t) == 5,
	"size_of_ae_enable_list_header_t_is_not_5_bytes");
...
_Static_assert(sizeof(struct nvme_mi_ae_occ_data) == 9,
	"size_of_nvme_mi_ae_occ_data_is_not_9_bytes");
...
_Static_assert(sizeof(struct nvme_mi_ae_occ_list_hdr) == 7,
	"size_of_nvme_mi_ae_occ_list_hdr_is_not_7_bytes");
 ...
 * updated with received size if successful
 *
 *
							...
							size_t *occ_list_num_bytes)
{
	...
	return nvme_mi_mi_config_set_async_event(ep,
										false,
										false,
										false,
										0,
...
/**
 * struct nvme_mi_aem_callbacks - Provided for nvme_mi_enable_aem
 * @aem_handler: Callback function for application processing of events
 * @enabled: Array indicating which AE should be enabled on the endpoint
 *
 * Application callbacks for nvme_mi_aem_process will be able to call
 * nvme_mi_aem_get_next_event which will return a pointer to such an identifier
 * for the next event the application should parse
...
/**
 * nvme_mi_get_aem_fd() - Returns the pollable fd for AEM data available
...
/**
 * nvme_mi_enable_aem() - Enable AE on the provided endpoint
 * @envfa: Enable SR-IOV virtual functions AE
 * @empfa: Enable SR-IOV physical functions AE
 * @encfa: Enable PCIe functions AE
 * @aemd: AEM Delay (time in seconds from when event happens to AEM being batched and sent)
 * @aerd: AEM Retry Delay (time in seconds between AEM retries from the endpoint)
 * @callbacks: AE configuraiton including which events are enabled and the callback function
 * @userdata: Application provided context pointer
 *
 ...
 *
 * Application should poll the fd that can be obtained from nvme_mi_get_aem_fd and then call
...
/**
 * nvme_mi_disable_aem() - Disable AE on the provided endpoint
 ...
 *
 * Call this if poll() indicates data is available on the fd provided by nvme_mi_get_aem_fd()
 *
 * This will call the fn pointer, aem_handler, provided with nvme_mi_aem_callbacks and the

...
static int nvme_mi_mctp_aem_purge(struct nvme_mi_ep *ep)
{
	...
	int flags = fcntl(mctp->sd_aem, F_GETFL, 0);

	fcntl(mctp->sd_aem, F_SETFL, flags | O_NONBLOCK);

	// Read until there is no more data
	while ((bytes_read = recv(mctp->sd_aem, buffer, sizeof(buffer), 0)) > 0)
		;

	// Check for errors other than EAGAIN or EWOULDBLOCK
	if (bytes_read < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
		perror("recv");

	// Restore the socket to blocking mode
	fcntl(mctp->sd_aem, F_SETFL, flags);
...
nvme_mi_ep_t nvme_mi_open_mctp(nvme_root_t root, unsigned int netid, __u8 eid)
{
	...
	//Skip this for now for test coverage
	//because it messes with the test_peer SD
	if (!(mctp->net == 0 && mctp->eid == 0)) {
		mctp->sd_aem = ops.socket(AF_MCTP, SOCK_DGRAM, 0);
		if (mctp->sd_aem < 0) {
			errno_save = errno;
			goto err_close_sd;
		}
		...
		if (bind(mctp->sd_aem, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
			errno_save = errno;
			close(mctp->sd_aem);
			goto err_close_sd;
		}

...
static int aem_sync(nvme_mi_ep_t ep,
	...
	__u8 aemd,
	__u8 aerd,
	struct ae_enable_item_t *items,
	__u8 num_items,
	struct nvme_mi_ae_occ_list_hdr *resp,
	size_t *resp_len
)
{
	size_t msg_len =
		sizeof(struct ae_enable_list_header_t) +
		num_items * sizeof(struct ae_enable_item_t);
	...
	//Data follows header
	struct ae_enable_item_t *msg_items = (struct ae_enable_item_t *)(request + 1);
...
static int aem_disable_enabled(nvme_mi_ep_t ep)
{
    ...
    // disable
	int rc = nvme_mi_aem_get_enabled(ep, already_enabled, sizeof(already_enabled));
	...
	struct ae_enable_item_t sync_data[NUM_ENABLES] = {0};

	for (int i = 0; i < NUM_ENABLES; i++) {
		if (already_enabled[i]) {
	...
	size_t len)
{
	...
	memset(enabled, 0, 256 * sizeof(*enabled));

	struct ae_enable_item_t *items = (struct ae_enable_item_t *)(enabled_list + 1);

	for (int i = 0; i < enabled_list->hdr.numaes; i++)
		enabled[items[i].aeei.aeeid] = items[i].aeei.aee;

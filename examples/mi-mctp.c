...
static void show_chsc_flags(uint16_t chsc)
{
	static const struct {
		...
		{ NVME_MI_CHSC_NSSRO,	"NSSRO (NVM Subsystem Reset Occurred)" },
		{ NVME_MI_CHSC_CECO,	"CECO (Controller Enable Change Occurred)" },
		...
		{ NVME_MI_CHSC_CTEMP,	"CTEMP (Composite Temperature Change)" },
		...
		{ NVME_MI_CHSC_TCIDA,	"TCIDA (Telemetry Controller-Initiated Data Available)" },
	};
	unsigned int i;
...
{
	uint16_t chsc = (uint16_t)chs->chsc[0] | ((uint16_t)chs->chsc[1] << 8);

...
static void show_chscf_flags(uint16_t chscf)
{
	static const struct {
		uint16_t mask;
		const char *name;
	} chscf_bits[] = {
		{ NVME_MI_CHSCF_RDY,	"RDY (Ready)" },
		{ NVME_MI_CHSCF_CFS,	"CFS (Controller Fatal Status)" },
		{ NVME_MI_CHSCF_SHST,	"SHST (Shutdown Status)" },
		{ NVME_MI_CHSCF_NSSRO,	"NSSRO (NVM Subsystem Reset Occurred)" },
		{ NVME_MI_CHSCF_CECO,	"CECO (Controller Enable Change Occurred)" },
		{ NVME_MI_CHSCF_NAC,	"NAC (Namespace Attribute Changed)" },
		{ NVME_MI_CHSCF_FA,	"FA (Firmware Activated)" },
		{ NVME_MI_CHSCF_CSTS,	"CSTS (Controller Status Change)" },
		{ NVME_MI_CHSCF_CTEMP,	"CTEMP (Composite Temperature Change)" },
		{ NVME_MI_CHSCF_PDLU,	"PDLU (Percentage Used Change)" },
		{ NVME_MI_CHSCF_SPARE,	"SPARE (Available Spare Change)" },
		{ NVME_MI_CHSCF_CWARN,	"CWARN (Critical Warning Change)" },
		{ NVME_MI_CHSCF_TCIDA,	"TCIDA (Telemetry Controller-Initiated Data Available)" },
	};
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(chscf_bits); i++) {
		if (chscf & chscf_bits[i].mask)
			printf("        flag: %s\n", chscf_bits[i].name);
	}
}
...
{
	uint16_t chscf = (uint16_t)chs->chscf[0] |
...
static int do_controllers(nvme_mi_ep_t ep)
{
	...
	if (chs) {
		rc = nvme_mi_mi_controller_health_status_poll_all(ep, true, chs, &num_health);
		if (rc) {
			warn("can't perform Controller Health Status Poll operation");
			free(chs);
			chs = NULL;
			num_health = 0;
		}
